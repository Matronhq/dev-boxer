...
class WizardTest < Minitest::Test
  def test_wizard_writes_config_and_secrets
    ...
    Dir.mktmpdir do |dir|
      ...
      refute_includes wizard_output, "GitHub access"
      refute_includes wizard_output, "GitHub personal access token"
      assert_nil secrets.dig("github", "token")
      ...
      assert_includes wizard_output, "name=Dev+Boxer+DNS+%28example.com%29"
      assert_includes wizard_output, "the name 'Dev Boxer DNS (example.com)'"
      ...
      assert_includes wizard_output, "two Zero Trust Access apps"
      assert_includes wizard_output, "bypass app that lets matrix"
      assert_includes wizard_output, "public-`"
  ...
  def test_wizard_can_choose_manual_tunnel_setup
    ...
    Dir.mktmpdir do |dir|
      ...
      assert_includes output.string, "Create the two Access apps yourself"
      ...
      assert_includes output.string, "It won't create the Access apps either"
      assert_includes output.string, "set up both"
      assert_empty DevBoxer::Config.validation_errors(DevBoxer::Config.load(config_path))
    end
  end
  ...
  def answers_for_there_branch(blob:, trailing_blob_retries: [], matrix_username: "alice")
    ([
      ...
      matrix_username,                            # 12. Your Matrix username on the external homeserver
